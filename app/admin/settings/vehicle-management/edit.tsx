...
// // console.log(vehicleData.rc_document,'userPhoss')
//     useEffect(() => {
//       if (vehicleData) {
...
//         };
//         reader.readAsDataURL(formData.userfile); // Set the image from userfile
//       }
//     }, [formData?.userfile]);
...
//     if (data.success) {
//       setSuccess(true);
//       onSave(formData); // Notify parent of updated data
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
    ...
    if (imageChanged && formData.userfile) {
      ...
      const response = await fetch(formData.rc_document);
      const blob = await response.blob();
      const rcFile = new File([blob], '', { type: blob.type }); // Give a filename
      formDataToSend.append('userfile', rcFile);
    } else if (formData.rc_document instanceof File) {
      // Directly append the File if rc_document is already a File object
    ...
    // setError('An error occurred while updating the vehicle.');
    toast.error(err.message || 'An error occurred while updating the vehicle.');
  } finally {
    setLoading(false);
  }
};
