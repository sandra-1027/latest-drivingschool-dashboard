...
import Add from './add';
import { FaArrowDown, FaArrowUp, FaRegCheckCircle } from 'react-icons/fa';
...
const page = () => {
  ...
  const fetchStaffData = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/staff/accounts/accounts_details', {
        method: 'POST',
        headers: {
  ...
  const totalPages = Math.ceil(totalEntries / entriesPerPage);
    ...
    const updateAccountStatus = async (id: string, status: string) => {
      try {
        const response = await fetch('/api/staff/accounts/inactivate_accounts', {
          method: 'POST',
          headers: {
            'authorizations': state?.accessToken ?? '', 
...
<form>
  ...
  <div className="col-span-4 sm:col-span-1 card px-3 pb-4 sm:px-5 pt-10">
  ...
  <table className="is-hoverable w-full text-left">
            ...
            {currentEntries.map((item, index) => (
              <tr key={item.id} className="border-y border-transparent border-b-slate-200 dark:border-b-navy-500">
                ...
                {/* <span class="badge bg-danger-transparent fw-medium text-capitalize">expense <i class="ri-arrow-down-fill"></i></span> */}

                <td className="whitespace-nowrap px-4 py-3 sm:px-5">
                {/* {item.daily_status} */}
                {item.daily_status === "income" ? (
    // <>
    //   <span className="flex">Income <HiOutlineArrowNarrowUp className='mt-1'/></span>
    // </>
    ...
    <>
      {/* <span className="flex">Expense <HiOutlineArrowNarrowDown className='mt-1'/> </span> */}
